...
exports.injectTapdSkill = injectTapdSkill;
exports.formatSkillResult = formatSkillResult;
 ...
 */
function backupSkillDir(skillDir) {
...
function promptForToken() {
    return new Promise(resolve => {
        ...
        logger_1.logger.info('  Get yours at: https://www.tapd.cn/tapd_api_token/token');
 ...
 */
function writeTokenConfig(skillDir, token) {
 ...
 */
function hasTokenConfig(skillDir) {
    try {
 ...
 */
function copySkillFiles(targetDir, force) {
    const filesWritten = [];
    const filesSkipped = [];
    const warnings = [];
    ...
    for (const filename of SKILL_ASSET_FILES) {
        ...
        if (content === null) {
            warnings.push(`Asset file "${filename}" not found in asset directory (${assetDir}) ` +
 ...
 */
async function injectTapdSkill(claudeInfo, opts = {}) {
    const { force = false, promptForToken = true, token: preToken = undefined, } = opts;
    const warnings = [];
    ...
    if (alreadyExists && force) {
        try {
            ...
            return {
                ...
                summary: `Failed to back up existing skill directory: ${err.message}`,
                warnings,
            };
        }
    }
    ...
    try {
        ...
        return {
            ...
            filesSkipped: [],
            tokenConfigured: false,
            summary: `Failed to copy skill files: ${err.message}`,
            warnings,
        };
    }
    ...
    if (!tokenConfigured) {
        let resolvedToken = preToken ?? null;
        // Use env var if available
        if (!resolvedToken) {
            resolvedToken = process.env['TAPD_API_TOKEN'] ?? null;
            if (resolvedToken) {
                logger_1.logger.info('[tapd-api] Using TAPD_API_TOKEN environment variable.');
            }
        }
        ...
        if (resolvedToken) {
            try {
                ...
                logger_1.logger.ok('[tapd-api] API token saved to config.json.');
            }
            catch (err) {
                warnings.push(`Could not write config.json: ${err.message}. ` +
                    `Run \`python ~/.claude/skills/tapd-api/setup.py\` manually.`);
            }
        }
        else {
            warnings.push('No TAPD API token configured. ' +
    ...
    return {
        ...
        tokenConfigured,
        summary: `tapd-api skill ${status} at ${skillDir}. ` +
